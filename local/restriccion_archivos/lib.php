...
/**
 * Observador de eventos para el plugin local_restriccion_archivos.
 */
class local_restriccion_archivos_observer {
    ...
    public static function file_created(\core\event\file_created $event) {
        ...
        $ext_permitidas = get_config('local_restriccion_archivos', 'extensiones_permitidas');
        $ext_prohibidas = get_config('local_restriccion_archivos', 'extensiones_prohibidas');
        ...
        if ($is_teacher) {
            ...
            if (in_array($extension, $ext_prohibidas) || (!empty($ext_permitidas) && !in_array($extension, $ext_permitidas))) {
                ...
                throw new \moodle_exception('error_extension_no_permitida', 'local_restriccion_archivos');
            }
        }
    }
}